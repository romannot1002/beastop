    ...
    try {
        ...
        const beasts = nfts
            ...
            .map(nft => {
                ...
                // ONLY use Alchemy's cached/thumbnail URLs (these work!)
                let imageUrl = nft.image?.cachedUrl || nft.image?.thumbnailUrl;
                
                // Skip if it's a Google Storage URL
                if (imageUrl && imageUrl.includes('storage.googleapis.com')) {
                    imageUrl = null;
                }
                
                // If no cached image, use SVG with token number
                if (!imageUrl) {
                ...
                console.log(`Beast #${tokenId}: ${imageUrl.substring(0, 100)}...`);
